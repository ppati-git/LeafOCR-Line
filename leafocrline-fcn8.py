...
def fcn8_model(input_size=(512, 512, 3)):
    """FCN-8s implementation for binary segmentation"""
    ...
    score_pool3 = Conv2D(1, (1, 1), padding='same')(pool3)  # Score from pool3
    ...
    # Upsample by 2
    upscore_pool4 = Conv2DTranspose(1, (4, 4), strides=(2, 2), padding='same')(fuse_pool4)

    # Fuse with pool3
    fuse_pool3 = add([upscore_pool4, score_pool3])

    # Final upsampling by 8
    outputs = Conv2DTranspose(1, (16, 16), strides=(8, 8), padding='same', activation='sigmoid')(fuse_pool3)
...
class fcn8DataGenerator(Sequence):
...
def train_fcn8(train_image_dir, train_mask_dir, val_image_dir, val_mask_dir,
                 epochs=50, batch_size=4, use_focal_loss=True):
    """Train fcn8 model"""

    train_gen = fcn8DataGenerator(
    ...
    callbacks = [
        ...
        tf.keras.callbacks.CSVLogger('fcn8_training_log.csv')
    ]
...
def predict_image_fcn8(model_path, image_path, target_size=(512, 512)):
...
if __name__ == "__main__":
    ...
    model.save("final_fcn8_model.keras")
    print("Training completed! Model saved as 'final_fcn8_model.keras'")
